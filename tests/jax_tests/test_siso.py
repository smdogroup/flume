...
class FunctionAnalysis_analysis(Analysis):
    ...
    def _analyze_adjoint(self):
        ...
        xb += fb * (a*np.cos(a*x)+b*np.sin(b*x))
...
class FunctionAnalysis_wrapper(Analysis):
    ...
    def callable_analyze(self,x: list,a: list):
        """
        Private analysis method for the object that computes all outputs with the provided input variables and parameters to the object
        """
        # Compute the value of f given the variables and parameters
        f = jnp.sin(x[0]*a[0])-jnp.cos(x[0]*a[1])
...
class TestAnalysisObject(unittest.TestCase):
    ...
    def test_set_var_values(self):
        ...
        self.test_obj_1.set_var_values(variables={"x": self.x})
        self.test_obj_2.set_var_values(variables={"x": self.x})
    ...
    def test_adjoint_values(self):
        ...
        Checks the computation of the output value for the analytic function
        """
        self.test_obj_2.declare_design_vars(variables=["x"])
        rel_error = self.test_obj_2.test_isolated_adjoint(debug_print=True)
