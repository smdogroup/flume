...
class TestAnalysisObject(unittest.TestCase):
    ...
    def test_parameter_values(self):
        ...
        for i, name in enumerate(param_names):
            with self.subTest(f"parameter = {name}"):
                self.assertEqual(
                    param_vals[i],
                    self.test_obj.parameters[name],
                    f"Values for parameter {name} do not match",
                )
    ...
    def test_isolated_adjoint(self):
        ...
        rel_error = self.test_obj.test_isolated_adjoint(
            print_res=False,
            debug_print=False,
            method="cs",
            defined_vars={"test.x": 2.0},
        )
    ...
    def test_combined_adjoint(self):
        ...
        # Perform the combined adjoint test for the object
        rel_error = self.test_obj.test_combined_adjoint(
            print_res=False,
            debug_print=False,
            method="cs",
            defined_vars={"test.x": 2.0},
        ...
        self.assertLessEqual(
            rel_error,
            1e-5,
            "Relative error for the combined adjoint test is greater than 1e-5.",
        )
