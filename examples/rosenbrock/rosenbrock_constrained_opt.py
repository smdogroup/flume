# Description: this defines the run script that combines the individual Analysis classes to perform optimization for the constrained Rosenbrock problem.

from flume.base_classes.system import System
from icecream import ic
import numpy as np
import argparse
from .rosenbrock_problem_classes import Rosenbrock, RosenbrockConstraint, RosenbrockDVs
...
if __name__ == "__main__":
    ...
    rosenbrock_con = RosenbrockConstraint(obj_name="con", sub_analyses=[rosenbrock_dvs])
    ...
    sys = System(
        ...
        log_name="flume.log",
        log_prefix="examples/rosenbrock/rosenbrock_constrained",
    )
    ...
    sys.declare_constraints(global_con_name={"con.g": {"direction": "leq", "rhs": 1.0}})
    ...
    xerr = (xstar[0] - x[0]) / xstar[0]
    yerr = (xstar[1] - x[1]) / xstar[1]

    print("\n%15s %15s %15s %15s" % ("Variable", "Expected", "Computed", "Rel. Error"))
    print("%15s %15s %15s %15s" % ("x", xstar[0], x[0], xerr))
    print("%15s %15s %15s %15s" % ("y", xstar[1], x[1], yerr))
